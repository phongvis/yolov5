...
from utils.metrics import ap_per_class, compute_f1, ConfusionMatrix
...
def run(data,
    ...
    if len(stats) and stats[0].any():
        p, r, ap, f1, optimal_conf, ap_class = ap_per_class(*stats, plot=plots, save_dir=save_dir, names=names)
...
def compute_metrics(data,
    ...
    if len(stats) and stats[0].any():
        ...
        if (verbose or (nc < 50)) and nc > 1 and len(stats):
            for i, c in enumerate(ap_class):
                print(pf % (names[c], seen, nt[c], p[i], r[i], f1[i]))

        metrics = [(_f1, _p, _r, names[_c]) for _f1, _p, _r, _c in zip(f1, p, r, ap_class)]
